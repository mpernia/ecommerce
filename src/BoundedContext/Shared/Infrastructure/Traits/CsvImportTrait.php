...
namespace Ecommerce\BoundedContext\Shared\Infrastructure\Traits;
...
{
    public function processCsvImport(Request $request): Application|Redirector|RedirectResponse|ApplicationAlias
    {
        try {
            ...
            $model     = sprintf('%s\%s', config('setting.models_dir'), $modelName);
    ...
    public function parseCsvImport(Request $request)
    {
        ...
        $fullModelName = sprintf('%s\%s', config('setting.models_dir'), $modelName);
