...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.preference.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("backoffice.preferences.update", [$preference->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('sort_product_by') ? 'is-invalid' : '' }}" name="sort_product_by" id="sort_product_by">
                    ...
                    @foreach(App\Models\Preference::SORT_PRODUCT_BY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('sort_product_by', $preference->sort_product_by) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('notify_method') ? 'is-invalid' : '' }}" name="notify_method" id="notify_method">
                    ...
                    @foreach(App\Models\Preference::NOTIFY_METHOD_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('notify_method', $preference->notify_method) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
