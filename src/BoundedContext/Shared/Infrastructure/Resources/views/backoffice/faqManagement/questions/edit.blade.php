...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("backoffice.faq-questions.update", [$faqQuestion->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                <label for="order">{{ trans('cruds.faqQuestion.fields.order') }}</label>
                <input class="form-control {{ $errors->has('order') ? 'is-invalid' : '' }}" type="number" name="order" id="order" value="{{ old('order', $faqQuestion->order) }}" step="1">
                @if($errors->has('order'))
                    <span class="text-danger">{{ $errors->first('order') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.faqQuestion.fields.order_helper') }}</span>
            </div>
            <div class="form-group">
