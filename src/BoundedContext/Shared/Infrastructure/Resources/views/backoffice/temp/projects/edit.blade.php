...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("backoffice.projects.update", [$project->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                <label class="required" for="advertiser_id">{{ trans('cruds.project.fields.client') }}</label>
                <select class="form-control select2 {{ $errors->has('client') ? 'is-invalid' : '' }}" name="advertiser_id" id="advertiser_id" required>
                    @foreach($clients as $id => $entry)
                        <option value="{{ $id }}" {{ (old('advertiser_id') ? old('advertiser_id') : $project->client->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('client'))
                    <span class="text-danger">{{ $errors->first('client') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.project.fields.advertiser_helper') }}</span>
            </div>
            <div class="form-group">
