...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("backoffice.leads.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('affiliate_campaign') ? 'is-invalid' : '' }}" name="affiliate_campaign_id" id="affiliate_campaign_id" required>
                    @foreach($affiliate_campaigns as $id => $entry)
                        <option value="{{ $id }}" {{ old('affiliate_campaign_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tracking') ? 'is-invalid' : '' }}" type="text" name="tracking" id="tracking" value="{{ old('tracking', '') }}" required>
