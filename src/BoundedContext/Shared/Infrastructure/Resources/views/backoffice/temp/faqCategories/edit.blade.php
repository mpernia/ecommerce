...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("backoffice.faq-categories.update", [$faqCategory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('order') ? 'is-invalid' : '' }}" type="number" name="order" id="order" value="{{ old('order', $faqCategory->order) }}" step="1">
                @if($errors->has('order'))
