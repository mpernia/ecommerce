...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.favoriteProduct.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("backoffice.favorite-products.update", [$favoriteProduct->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_active') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="is_active" value="0">
                    <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ $favoriteProduct->is_active || old('is_active', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $favoriteProduct->price) }}" step="0.01" required>
                @if($errors->has('price'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price_target') ? 'is-invalid' : '' }}" type="number" name="price_target" id="price_target" value="{{ old('price_target', $favoriteProduct->price_target) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id">
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $favoriteProduct->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
