...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("backoffice.content-categories.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                <label for="icon">{{ trans('cruds.contentCategory.fields.icon') }}</label>
                <input class="form-control {{ $errors->has('icon') ? 'is-invalid' : '' }}" type="text" name="icon" id="icon" value="{{ old('icon', '') }}">
                @if($errors->has('icon'))
                    <span class="text-danger">{{ $errors->first('icon') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.contentCategory.fields.icon_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="parent_id">{{ trans('cruds.contentCategory.fields.parent') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.contentCategory.fields.parent_helper') }}</span>
            </div>
            <div class="form-group">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('backoffice.content-categories.storeCKEditorImages') }}', true);
